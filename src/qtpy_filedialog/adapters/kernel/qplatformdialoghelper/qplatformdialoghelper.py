...
from qtpy_filedialog.adapters.kernel.qplatformdialoghelper.qfiledialogoptions import (
    QFileDialogOptions,
)
    ...
    def __new__(cls, *args, **kwargs) -> Self:  # type: ignore[name-defined]
        ...
        if cls is QPlatformDialogHelper:
            if sys.platform in ("win32", "cygwin"):
                from qtpy_filedialog.adapters.kernel.qplatformdialoghelper.qwindowsdialoghelpers import (
                    QWindowsDialogHelper,
                )

                new_cls = QWindowsDialogHelper
            elif sys.platform == "linux":
                from qtpy_filedialog.adapters.kernel.qplatformdialoghelper.qlinuxdialoghelpers import (
                    LinuxFileDialogHelper,
                )

                new_cls = LinuxFileDialogHelper
            elif sys.platform == "darwin":
                from qtpy_filedialog.adapters.kernel.qplatformdialoghelper.qmacdialoghelpers import (
                    QMacDialogHelper,
                )
